...
| Field | Value |
| --- | --- |
| Task-suite version | 1.0.5 (PATCH: `gemini_tool_call_gap` resolves the client's tool-name spelling instead of summing both; see the changelog below) |
| Date | 2026-08-20 |
...
8. **The client renamed MCP tools in its usage output, which silently killed the tool-call gap.** `stats.tools.byName` keys moved from the bare server-side name to `mcp_<server>_<tool>`. Section 4.2 has the full account; the short version is that the gap's definition sums positive differences, the rename made every difference negative, and the detector would have read a clean zero forever while detecting nothing. The runner resolves the spelling per advertised tool, taking the qualified key when the client JSON carries one and the bare key otherwise. It summed the two counts instead until 1.0.5, which fixed the silent zero but admitted a false positive on a server advertising a name that collides with a client built-in, per item 2 above.
