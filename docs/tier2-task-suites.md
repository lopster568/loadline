...
| Field | Value |
| --- | --- |
| Task-suite version | 1.0.7 (PATCH: call-response tokens are also published with each result's top-level `_meta` envelope stripped; see the changelog below) |
| Date | 2026-08-20 |
...
**1.0.7, 2026-08-25. PATCH: `tool_call_result_tokens_meta_stripped` travels from the analyzer through the manifest into the summary.** Analyzer 0.2.0 counts each `tools/call` result twice, verbatim and with the top-level `_meta` member removed, because some servers ride an envelope there (github: the serverInfo block, icons included) that one client requests and another does not; the raw figure conflates what the server sent with what the protocol wrapped around it. Section 4 has the definition. Rows recorded before this version lack the field, which reads as not measured, never as zero. No prompt, fixture, success criterion or existing figure moves; every raw cell of the 2026-08-18 run reproduces exactly under the new analyzer before stripping.

**1.0.6, 2026-08-25. PATCH: `tool_call_gap_bare_unwired` is recorded per trial and aggregated per cell.** The 1.0.5 resolution rule removes the false positive but discards evidence when a trial holds both spellings of a served tool: a call attributed to a name that never appeared in a `tools/call` frame died inside the client, which is a different fact from a call the wire lost, and one integer cannot carry both. The new field keeps that count under its own label; `tool_call_gap` keeps its name, meaning and published values, and the new field does not set `harness_suspect`. Checked against the corpus before shipping: no trial in it holds both spellings for the same tool, so the field reads zero everywhere today and the change removes a guess from the detector before the guess has to be made. No prompt, fixture or success criterion moves, so every 1.0.5 result stays comparable across the bump.

**1.0.5, 2026-08-24. PATCH: `gemini_tool_call_gap` resolves the client's tool-name spelling per advertised tool instead of summing both spellings.** Summing was the 1.0.4 rule and it admitted a false positive on one server; section 4.2 has the account. No prompt, fixture or success criterion moves, so every 1.0.4 result stays comparable across the bump, and no published gap changes value: recomputed over every Gemini trial in the corpus, the two formulas agree on all of them.
...
| Tokens per completed task, call-response | Tokens consumed by the actual `tools/call` request and response traffic during the task | Computed from the interposer JSONL: `params_full` of each `tools/call` request plus `result_summary` (or `result_full` under `--full-results`) of each response, counted with the `o200k_base` tokenizer per `methodology-v0.md` 1.6, so the number is comparable to Tier 1's token basis. Frame-derived for BOTH clients, never read from either client's usage output: a frame count measures the same object regardless of how a client accounts internally, and no published call-token figure reads `client_reported` or `cache` | MEASURED |
| Tokens per completed task, call-response, `_meta`-stripped | The same traffic with each result's top-level `_meta` member removed before counting | Analyzer 0.2.0 (suite 1.0.7), same frames, same tokenizer. Some servers ride a protocol envelope in `_meta` (github sends Claude Code a serverInfo block of roughly 1,400 tokens on every response and sends Gemini CLI none), so the raw figure conflates server payload with envelope; this column isolates the payload. Only the top level is stripped, because that is where the envelope rides; a `_meta` nested deeper is the server's own payload and stays. A result with no top-level `_meta` counts identically in both columns | MEASURED |
...
**The measure is only as good as the name matching, and the client upgrade broke it silently once.** The wire name in the log is the server's own bare tool name, because that is what travels in a `tools/call` frame. The client keys its usage output by the name the *model* called, and that spelling changed between the two Gemini versions: 0.18.4 used the bare name, 0.55.1 uses the fully qualified `mcp_<server>_<tool>`. Matched on the bare name alone, every advertised tool scored zero client calls against a positive wire count, every difference came out negative, and the sum-of-positive-differences definition above discarded all of them. The gap would have read a clean zero on every trial while detecting nothing. It is worth stating plainly because of the shape of the failure rather than its size: a detector that reports the healthy value when it is broken cannot be distinguished from a detector that is working, and this one is the only thing standing between a client-side interop fault and a published capability number. The runner therefore resolves the spelling per advertised tool: it takes the qualified `mcp_<server>_<tool>` count when the trial's own client JSON carries that key, and the bare count otherwise.

**Resolving beats summing, because summing reopens the hole at the other end.** Summing both spellings was the 1.0.4 rule and it fixed the silent zero, but it admits a false positive on any server that advertises a name colliding with a client built-in. Section 7.1 item 2 records the collision for its own reason: the filesystem server's surface overlaps Gemini's built-ins on `read_file`, `write_file` and `list_directory`. Under `tools.core: ["mcp_*"]` none of those bare names is registered, so a call the model forms to bare `write_file` dies inside the client exactly the way `run_shell_command` does, lands in `stats.tools.byName`, and then survives the advertised-name filter, because the server does advertise that name. The filter has nothing to drop when the stray *is* an advertised name. On 0.18.4 that stray shared a counter with the real calls and had wire frames to cancel against; on 0.55.1 the real call is keyed `mcp_filesystem_write_file` and the stray is keyed `write_file`, so adding them guarantees no cancellation and the term goes positive on a trial where nothing was lost on the pipe. The two keys are different evidence and summing throws the distinction away. Exposure is one server wide, and the false positive never fired: across the 45 Gemini trials of the 1.0.1 run, no bare stray was ever a name its own server advertised, which is the only reason every published gap is a true zero. The shape is nonetheless already in the data. Fourteen of those 45 trials carry at least one bare stray in `stats.tools.byName` (`update_topic`, `read_file`, `run_shell_command`, `grep_search`), and four of them are filesystem trials, on the one server whose surface collides. Those four stranded `update_topic`, which filesystem does not advertise. Nothing about the run makes that the general case.

**The residual, stated rather than hidden.** On 0.55.1, if a call to a served tool is keyed bare with no qualified key present for that tool, the bare count is still used, and a stray in that shape would still read positive. That is one trial-shape wide and it did not occur on any of the run's 45 Gemini trials. Any future change to how a client names a server's tool has to be checked against this function, not assumed. All three spelling shapes (both keys present, qualified only, bare only) are asserted as unit cases by `tier2/run-suite.sh --self-test`, in both directions: the false positive the summing rule allowed, and the silent zero the name matching exists to catch.

**What resolution discards is kept as a second labeled field: `tool_call_gap_bare_unwired`** (suite 1.0.6). When a trial holds both spellings for a served tool, the qualified key wins the gap arithmetic and the bare count is dropped from it, but that count is not noise: it is the model reaching for a disabled built-in whose name the server also advertises, dying inside the client by design. Per served tool, the field counts the bare key only when the qualified key for that same tool is present, because with the qualified key present every wire frame for the tool is attributed to the qualified spelling, which is what makes the bare count unwired by construction. Two deliberate gates prevent double-reporting. On a trial with no `mcp_<srv>_` key at all, the client is keying by bare names, the bare count is the real count, and the field reads zero. And in the residual shape above, a served tool keyed bare with no qualified key present, the bare count is already `tool_call_gap` fallback evidence and is not repeated here. A positive value does not set `harness_suspect`: the interop-fault signal stays with `tool_call_gap`, and a reader can now separate a client-side MCP interop fault from a built-in collision with neither able to hide the other. Checked against every Gemini trial in the corpus before shipping: no trial holds both spellings for the same tool, so the field reads zero everywhere today. This is not fixing an observed wrong number; it is removing a guess from a detector before the guess has to be made.
...
8. **The client renamed MCP tools in its usage output, which silently killed the tool-call gap.** `stats.tools.byName` keys moved from the bare server-side name to `mcp_<server>_<tool>`. Section 4.2 has the full account; the short version is that the gap's definition sums positive differences, the rename made every difference negative, and the detector would have read a clean zero forever while detecting nothing. The runner resolves the spelling per advertised tool, taking the qualified key when the client JSON carries one and the bare key otherwise. It summed the two counts instead until 1.0.5, which fixed the silent zero but admitted a false positive on a server advertising a name that collides with a client built-in, per item 2 above. Since 1.0.6 the bare count that resolution drops is recorded under its own label, `tool_call_gap_bare_unwired`; section 4.2 has the design and its two anti-double-reporting gates.
